...
public class WaybackCommand {
    ...
    private static void conf(String[] arg, CommandSender sender) {
        if (arg.length == 0) throw new NullPointerException(Language.asString("COMMANDS.ILLEGAL_ARGUMENT"));
        else switch (arg[0]) {
            case "decrypt":
            case "dec":
            ...
            case "enc":
            case "encrypt":
                if (arg.length < 2) throw new NullPointerException(Language.asString("INPUT_PASSWORD"));
    ...
    private static void task(String[] arg, CommandSender sender) {
        if (arg.length == 0) throw new NullPointerException(Language.asString("COMMANDS.ILLEGAL_ARGUMENT"));
            ...
            Language.sendTo(sender, "TASKS.LIST", String.valueOf(WaybackConf.getConf().getPoolSize()));
            printRunning();
            WaybackSchedules.instance().getPending().stream()
                    .filter(task -> !task.isRunning()).forEach(task -> {
                if (task instanceof DelayedSchedule) Language.sendTo(sender, "TASKS.DELAYING_TASK_FORMAT",
                    ...
                    if (((PeriodSchedule) task).getLastRun() != 0)
                        Language.sendTo(sender, "TASKS.PERIOD_TASK_FORMAT",
                                task.id(), task.name(), ((PeriodSchedule) task).timeToRun(), ((PeriodSchedule) task).lastRun());
                    else Language.sendTo(sender, "TASKS.PERIOD_TASK_FORMAT2",
                            task.id(), task.name(), ((PeriodSchedule) task).timeToRun());
                } else Language.sendTo(sender, "TASKS.PENDING_TASK_FORMAT", task.id(), task.name());
            });
        }
    }
    ...
    private static void reload(String[] arg, CommandSender sender) {
        if (getSchedules().getRunning().size() > 0) {
            Language.sendTo(sender, "RELOAD_TASK_RUNNING");
            return;
        }
        if (Wayback.reload()) {
            Language.sendTo(sender, "RELOAD_SUCCESS");
        } else {
            Language.sendTo(sender, "RELOAD_FAILED");
        }
    }
    ...
    private static void debug(String[] arg, CommandSender sender) {
        ...
        Language.sendTo(sender, "COMMANDS.DEBUG_SWITCH", String.valueOf(Wayback.logger().getLevel()));
    }
    ...
    private static void rollback(String[] arg, CommandSender sender) {
        if (arg.length == 0) Language.sendTo(sender, "ROLLBACK.HELP");
        else if ("list".equalsIgnoreCase(arg[0])) {
            if (arg.length >= 2)
            ...
            Language.sendToConsole("ROLLBACK.PREPARE");
            Bukkit.getPluginManager().registerEvents(new RejectJoiningListener(), Wayback.instance());

            Language.sendToConsole("ROLLBACK.PREPARE_KICK_PLAYERS");
            Bukkit.getWorlds().stream().flatMap(world -> world.getPlayers().stream())
                    .forEach(player -> player.kickPlayer(Language.asString("ROLLBACK.KICK_ROLLBACK")));
            ...
            switch (split.length) {
                ...
                case 5:
                    ...
                    break;
                default:
                    Language.sendTo(sender, "ROLLBACK.ROLLBACK_DATE_ERROR");
                    return;
            }
            WaybackConf.getConf().getStorages().values().stream()
                    ...
                    .ifPresent(breakpoint -> {
                        ...
                        Language.sendTo(sender, "COMMANDS.CONFIRM_BACKUP", breakpoint.getTime().toString());
                    });
        }
    }
    ...
    private static final class RejectJoiningListener implements Listener {
        ...
        public void onConnect(PlayerLoginEvent event) {
            ...
            event.setKickMessage(Language.asString("ROLLBACK.KICK_ROLLBACK"));
        }
